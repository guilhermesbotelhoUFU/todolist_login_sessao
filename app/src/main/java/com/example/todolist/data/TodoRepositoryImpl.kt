...
class TodoRepositoryImpl(
    ...
    override suspend fun insert(title: String, description: String?, id: Long?, idUser: String?) {
        val entity = id?.let {
            dao.getBy(it)?.copy(
                title = title,
    ...
    override fun getAll(idUser: String?): Flow<List<Todo>> {
        return dao.getAll(idUser).map { entities ->
            entities.map { entity ->
                Todo(
                    id = entity.id,
                    title = entity.title,
